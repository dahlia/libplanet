...
namespace Libplanet.Net
{
        ...
        where TAction : IAction, new()
    {
        ...
        EnumerateChunks(CancellationToken cancellationToken = default) =>
            new AsyncEnumerable<IEnumerable<HashDigest<SHA256>>>(async yield =>
            {
                ...
                while (!(cancellationToken.IsCancellationRequested || QueuedDemandCompleted()))
                {
                    _logger.Verbose(
                    ...
                    await _demandEnqueued.WaitAsync(1000, cancellationToken);
        ...
        )
        {
            ...
            System.Collections.Async.IAsyncEnumerable<IEnumerable<HashDigest<SHA256>>> chunks =
                EnumerateChunks(cancellationToken);

            return new AsyncEnumerable<Block<TAction>>(async yield =>
            {
                await chunks.ForEachAsync(
                    async hashes =>
                    {
